...
function fqBrand(id){
	//判断是否有价格
	if(price != ''){//页面需要刷新用 window.location.href
			...
			<div class="sl-b-selected J_brandSelected">
				<span class="crumbs-arrow">已选条件：</span>
					<c:forEach items="${map }" var="m">
						<a title="依琦莲（yiqilian）" onclick="alert(52)"  href="javascript:;" class="crumb-select-item">
...
<c:if test="${empty brandId }"> <%--如果品牌ID是空则隐藏--%>
...
<div id="J_goodsList" class="goods-list-v1 gl-type-1 J-goods-list">
	<ul class="gl-warp clearfix" data-tpl="1">
		...
		<li data-sku="1711416562" class="gl-item">
			<div class="gl-i-wrap">
				<div class="p-img">   <%--window.open() 点击去商品详情界面,需要打开新的窗口--%>
				...
				<div class="p-name p-name-type-2">
					<a title="满129立减10,199减20优惠券,支持货到付款" href="javascript:;" onclick="window.open('http://localhost:8083/html/product/${product.id}.html')"> <!--加载静态化页面-->
						<em>${product.name }</em>
					</a>
				</div>
				<div class="p-commit">
